...
public class WindowBase : Window
{
    ...
    private ComCtl32.SUBCLASSPROC windowSubclassProc = null!;
    ...
    protected async Task<bool> CheckProcessAsync(string? baseFolder = null)
    {
        ...
        var ps = Process.GetProcessesByName("DeepseekHarness").Where(p => p.Id != Environment.ProcessId).Where(x => x.MainModule?.FileName.StartsWith(baseFolder ?? "") ?? true);
        if (ps.Any())
        {
            bool close = await MessageBox.AskYesNoAsync(string.Format(Lang.AppIsRunningForceClose, string.Join(",", ps.Select(x => x.Id))), PromptIconKind.Warning, owner: this);
            if (close)
            {
                foreach (var p in ps)
                {
